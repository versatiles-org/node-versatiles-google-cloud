	...
	describe('loadConfig', () => {
		it('throws error when file not found', async () => {
			await expect(loadConfig('/nonexistent/path/config.yaml')).rejects.toThrow(
				/Failed to read config file.*ENOENT/,
			);
		});
...
`,
		...
		it('supports multi-level inheritance', async () => {
			const grandparentPath = writeConfig(
				'grandparent.yaml',
				`
...
`,
	...
	describe('type validation', () => {
		...
		it('throws error when localDirectory is not a string', async () => {
			...
			await expect(loadConfig(path)).rejects.toThrow(/"localDirectory" must be a string/);
		});

		it('throws error when verbose is not a boolean', async () => {
			const path = writeConfig('invalid-verbose.yaml', 'verbose: 1');
			await expect(loadConfig(path)).rejects.toThrow(/"verbose" must be a boolean/);
		});
	});
	...
	describe('rewrite rules validation', () => {
		it('throws error when rewriteRules is not an object', async () => {
			...
			await expect(loadConfig(path)).rejects.toThrow(/"rewriteRules" must be an object/);
		});

		it('throws error when rewriteRules is an array', async () => {
			const path = writeConfig(
				'invalid-rules-array.yaml',
				`
rewriteRules:
  - ["/source", "/target"]
`,
...
`,
			...
			await expect(loadConfig(path)).rejects.toThrow(
				/rewriteRules\["\/source"\] value must be a string/,
			);
		});

		it('throws error when key does not start with /', async () => {
			const path = writeConfig(
				'invalid-rule-key.yaml',
				`
rewriteRules:
  "source": "/target"
`,
			...
			await expect(loadConfig(path)).rejects.toThrow(
				/rewriteRules key "source" must start with "\/"/,
			);
		});

		it('throws error when value does not start with /', async () => {
			const path = writeConfig(
				'invalid-rule-target.yaml',
				`
rewriteRules:
  "/source": "target"
`,
			...
			await expect(loadConfig(path)).rejects.toThrow(
				/rewriteRules\["\/source"\] value must start with "\/"/,
			);
		});

		it('validates all rules in object', async () => {
			const path = writeConfig(
				'invalid-second-rule.yaml',
				`
rewriteRules:
...
`,
			...
			await expect(loadConfig(path)).rejects.toThrow(
				/rewriteRules key "invalid" must start with "\/"/,
			);
		});

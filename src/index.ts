...
program
	...
	.option(
		'-b, --base-url <url>',
		'Set the public base URL. Defaults to "http://localhost:<port>/".',
	)
	.option(
		'-c, --config <path>',
		'Load configuration from a YAML file. CLI arguments override config file values.',
	)
	...
	.option(
		...
		'Ignore bucket and use a local directory instead. Useful for local development and testing.',
	)
	...
	.action(async (bucketName: string | undefined, cmdOptions: Record<string, unknown>) => {
		...
		if (verbose) {
			...
			console.table({
				...
				...Object.fromEntries(
					ruleEntries.map(([source, target], i) => [
						'rewriteRule ' + (i + 1),
						`${source} => ${target}`,
					]),
				),
